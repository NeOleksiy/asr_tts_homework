...
class LogMelFilterBanks(nn.Module):
    ...
    def forward(self, x):
        ...
        power_spectr = torch.abs(spectr) ** self.power
        mels = torch.matmul(power_spectr.transpose(-1, -2), self.mel_fbanks.to(x.device)).transpose(-1, -2)
        log_mel = torch.log(mels + 1e-8)
        return log_mel
